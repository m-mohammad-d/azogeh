...
function UpdateProduct() {
  ...
  const { data: product, isLoading: productLoading, error: productError } = useGetProductByIdQuery(productId as string);
  ...
  useEffect(() => {
    if (product) {
      ...
      setValue("image", product.data.product.image);
      setValue("brand", product.data.product.brand);
      setValue("category", product.data.product.category);
      setValue("countInStock", product.data.product.countInStock);
      setValue("description", product.data.product.description);
      setImageUrl(product.data.product.image);
    }
  }, [product, setValue]);
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      await UpdateProdct({ productId, ...productNewData }).unwrap();
      toast.success("محصول با موفقیت اپدیت شد!");
    } catch (error) {
      toast.error("خطا در اپدیت محصول:");
    }
  };
  ...
  if (productError) return <p>مشکلی پیش امده است</p>;
